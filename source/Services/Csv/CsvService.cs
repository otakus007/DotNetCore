...
public class CsvService : ICsvService
{
    ...
    {
        if (items is null || !items.Any()) return new MemoryStream();
        ...
        items.ToList().ForEach(async item => await streamWriter.WriteLineAsync(string.Join(separator, properties.Select(property => property.GetValue(item)))));
